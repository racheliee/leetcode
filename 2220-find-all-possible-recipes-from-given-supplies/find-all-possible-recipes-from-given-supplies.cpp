class Solution {
public:
    bool dfs(string food, const unordered_map<string, vector<string>>& adj, unordered_set<string>& supply_set, unordered_map<string, int>& visited) {
        cout << "food: " << food << endl;
        if(supply_set.find(food) != supply_set.end())            
            return true;
        else {
            if(adj.find(food) == adj.end()) // not in supply or recipes; impossible
                return false;
        }
    ...
    vector<string> findAllRecipes(vector<string>& recipes, vector<vector<string>>& ingredients, vector<string>& supplies) {
        unordered_set<string> supply_set = unordered_set(supplies.begin(), supplies.end());
